...
int fat16_list_dir(const char* path) {
    ...
    vesa_print("\n  Directory of /");
    if(path&&path[0]&&!(path[0]=='/'&&path[1]=='\0')){vesa_print(path);}
    vesa_print("\n\n");
        ...
        for(uint32_t s=0;s<root_dir_sectors;s++){
            ...
            for(uint32_t i=0;i<SECTOR_SIZE/32;i++){
                ...
                if(E[i].attributes&0x10){
                    ...
                    vesa_print("  ");vesa_print(fn);
                    for(int p=strlen(fn);p<14;p++)vesa_print_char(' ');
                    vesa_print("<DIR>\n");
                } else {
                    int fl=strlen(fn);
                    ...
                    vesa_put_dec(E[i].file_size);vesa_print(" bytes\n");
                }
                count++;
            }
        }
root_ls_done:;
        vesa_set_color(COLOR_WHITE,COLOR_BLACK);
        vesa_print("\n  "); vesa_put_dec(count); vesa_print(" item(s)\n\n");
        return count;
    }
